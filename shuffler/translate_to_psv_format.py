"""
shuffler/translate_to_psv_format.py — Map synthetic comment columns to the
canonical export schema and write the result as a ♔-delimited PSV file.

Input  : ♔-delimited synthetic comment file produced by syncom/export.py
Output : ♔-delimited PSV file (same schema as the real docket CSV export)

The output columns exactly match the docket CSV export so that synthetic
...
def translate_synthetic_to_psv(input_file: str, output_file: str) -> int:
    """
    Translate a ♔-delimited syncom output file to a ♔-delimited PSV file
    whose columns match the docket export schema.
    ...
    output_file:
        Destination path for the translated PSV (e.g. synthetic.psv).
    ...
    # ── Map columns to regulations.gov schema ──────────────────────────────────────────
    cms_rows: list[dict] = []
    for row in rows:
...
def main() -> None:
    if len(sys.argv) < 2:
        ...
        print("=" * 50)
        print()
        print("Usage: python shuffler/translate_to_psv_format.py <input_file> [output_file]")
        print()
        print("Example:")
        print("  python shuffler/translate_to_psv_format.py \\")
        print("    CMS-2025-0050/synthetic_comments/synthetic.txt \\")
        print("    CMS-2025-0050/synthetic_comments/synthetic.psv")
        sys.exit(1)
    ...
    n = translate_synthetic_to_psv(input_file, output_file)
