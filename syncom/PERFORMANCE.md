...
### Files Modified
1. **`syncom/config.py`** - Added `async_openai_client()` and `async_embedding_client()`
2. **`syncom/generator.py`** - Added `generate_comment_async()`
3. **`syncom/persona.py`** - Added `sample_persona_async()` and `_generate_hook_async()`
4. **`syncom/argument_mapper.py`** - Added `map_argument_async()`
5. **`syncom/quality_control.py`** - Added `check_async()` with parallel QC checks
6. **`syncom/pipeline.py`** - Added `run_async()` with semaphore-controlled concurrency
